...
export function ActivityBlock({ title, id, items, className }) {
    return (
        <section id={id} className={`${styles.container} flex-1 max-w-lg ${className}`}>
            <div className={cnJoin([
                ...
                <h1 className='text-white font-display font-bold uppercase text-2xl'>{title}</h1>
            </div>
            <ul className={styles.itemsContainer}>
                {items.map((item) => (
                    <li key={item.url} className='block mt-4'>
                        <Link to={item.url} className={`${styles.item} bg-white flex group`}>
                            <div className='flex-grow p-4'>
                                <p className='font-display font-bold text-lg leading-tight mb-1'>{item.title}</p>
                                <p className='text-black font-display font-semilight text-xs'>{item.description}</p>
                            </div>
